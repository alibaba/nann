...
def difference(a, flags):
  values, row_splits, flags = tf.bloom_filter_difference(a.values, a.row_splits, flags, bucket=0, bucket_size=bucket_size)
  return tf.RaggedTensor.from_row_splits(values, row_splits, validate = validation), flags


with tf.device("/CPU:0"):
  flags = tf.Variable([0]*bucket_size) 
  a = tf.ragged.constant([[1,1,2,2,3,4,5],[11,12,13]], dtype=tf.int32)
  b = tf.ragged.constant([[4,5,6,7,7,8,10],[1000,13,14]], dtype=tf.int32)
