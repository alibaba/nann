"""
    https://github.com/poppinace/indexnet_matting/blob/master/scripts/hlvggnet.py
"""
import torch
import torch.nn as nn
import torch.nn.functional as F

__all__ = ['HLDeepMatting',
        ]
...
class HLDeepMatting(nn.Module):
    def __init__(self, input_chn, output_chn=1, pretrained=None):
        super(HLDeepMatting, self).__init__()
        ...
        self.alpha_pred = nn.Conv2d(64, output_chn, kernel_size=5, padding=2)
    ...
    def forward(self, x):
        ...
        x5d = self.unpool5(x6d, indices=idx5p, output_size=size5)
        x5d = F.relu(self.dconv5(x5d))

        x4d = self.unpool4(x5d, indices=idx4p, output_size=size4)
        x4d = F.relu(self.dconv4(x4d))

        x3d = self.unpool3(x4d, indices=idx3p, output_size=size3)
        x3d = F.relu(self.dconv3(x3d))

        x2d = self.unpool2(x3d, indices=idx2p, output_size=size2)
        x2d = F.relu(self.dconv2(x2d))

        x1d = self.unpool1(x2d, indices=idx1p, output_size=size1)
...
if __name__ == "__main__":
    net = HLDeepMatting(input_chn=4, output_chn=1, pretrained=None)