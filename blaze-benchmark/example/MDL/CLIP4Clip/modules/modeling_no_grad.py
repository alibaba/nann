...
from modules.module_clip_no_grad import CLIP, convert_weights
...
class CLIP4Clip(CLIP4ClipPreTrainedModel):
    ...
    def get_sequence_output(self, input_ids):
      with torch.no_grad():
        input_ids = input_ids.view(-1, input_ids.shape[-1])
    ...
    def get_sim_matrix_vta(self, text_features, artwork_features,  video, video_mask, shaped=False, video_frame=-1):
        if shaped is False:
            bs_pair = video_mask.size(0)
            visual_hidden_by_image, logits_per_text, logits_per_artwork = self.clip.forward_with_feature_clip_artwork(video, text_features, artwork_features)
        with torch.no_grad():
            visual_hidden = visual_hidden_by_image.view(bs_pair, -1, visual_hidden_by_image.size(-1))
            ...
            sim_matrix = (logit_scale * torch.matmul(query_features, visual_output.t()))

            sim_matrix = sim_matrix.view((sim_matrix.numel(), -1))
            raw_scores = sim_matrix / logit_scale
            return raw_scores.view(-1)
