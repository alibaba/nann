...
class ResidualAttentionBlockGrad(nn.Module):
    ...
    def attention(self, x: torch.Tensor):
      with nvtx.annotate("attention", color="red"):
        ...
        if self.set_grad:
            return self.attn(x, x, x, need_weights=False, attn_mask=attn_mask_, attention_probs_forward_hook=self.set_attn_probs,
                            attention_probs_backwards_hook=self.set_attn_grad)[0]
        else:
            return self.attn(x, x, x, need_weights=False, attn_mask=attn_mask_, attention_probs_forward_hook=None,
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask = None):
        ...
        # self.resblocks_grad = nn.Sequential(*[ResidualAttentionBlockGrad(width, heads, attn_mask) for _ in range(layers)])

    def forward(self, x: torch.Tensor, video_frame=-1):
      with nvtx.annotate("Transformer", color="red"):
...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, video_frame=-1):
      with nvtx.annotate("VisualTransformer", color="blue"):
        ...
        zeros = self.zero_map.get((x.shape[0], 1, x.shape[-1], x.dtype))
        if zeros is None:
          zeros = torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device)
          self.zero_map[(x.shape[0], 1, x.shape[-1], x.dtype)] = zeros
...
class CLIP(nn.Module):
    ...
    @staticmethod
    def get_config(pretrained_clip_name="ViT-B/32", model_path=None):
    ...
    def forward_with_feature(self, image, text):
        image_features = self.encode_image(image)
        text_features = self.encode_text(text)
        ...
        # shape = [global_batch_size, global_batch_size]
        return text_features, image_features, logits_per_image, logits_per_text

    def forward_with_text_artwork(self, text, artwork):
      with nvtx.annotate("encode artwork", color="blue"):
        artwork_features = self.encode_image(artwork)
        artwork_features = artwork_features / artwork_features.norm(dim=-1, keepdim=True)
      with nvtx.annotate("encode text", color="blue"):
        text_features = self.encode_text(text)
        text_features = text_features / text_features.norm(dim=-1, keepdim=True)
        return text_features, artwork_features

    def forward_with_image_text_feature(self, image, text_features):
      #with nvtx.annotate("encode image", color="blue"):
        # import pdb;pdb.set_trace()
        image_features = self.encode_image(image)
        # normalized features
        image_features = image_features / image_features.norm(dim=-1, keepdim=True)

      #with nvtx.annotate("logits", color="blue"):
        # cosine similarity as logits
        logit_scale = self.logit_scale.exp()
        logits_per_text = logit_scale * text_features @ image_features.t()
        # shape = [global_batch_size, global_batch_size]
        return  image_features, logits_per_text

    def forward_with_feature_clip_artwork(self, image, text_features, artwork_features):
      with nvtx.annotate("encode image", color="blue"):
        # import pdb;pdb.set_trace()
        image_features = self.encode_image(image)
        # normalized features
      ...
      with nvtx.annotate("logits", color="blue"):
        ...
        logit_scale = self.logit_scale.exp()
        logits_per_text = logit_scale * text_features @ image_features.t()
        logits_per_artwork = logit_scale * artwork_features @ image_features.t()
        # shape = [global_batch_size, global_batch_size]

        return  image_features, logits_per_text, logits_per_artwork
